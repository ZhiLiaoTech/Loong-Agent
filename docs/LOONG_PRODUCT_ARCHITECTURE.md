...
└────────────────────────────┬────────────────────────────────┘
                             │
┌────────────────────────────▼────────────────────────────────┐
│  Kernel 现有 @dragon/*（gateway · core · cli · memory …）   │
└─────────────────────────────────────────────────────────────┘
```
...
| 包名 | 职责 | 来源参考 |
|------|------|----------|
| `@dragon/client` | Gateway RPC/SSE、配置类型、连接与重试、Cloud API 封装 | ClawWorks `services/auth.ts`、`gateway.ts` 逻辑抽象 |
| `@dragon/host` | Host 能力接口（TS）、Browser 空实现 / Desktop 由 Tauri 注入 | ClawWorks `tauriCommands.ts` 分组 |
| `@dragon/ui` | Design tokens、布局、基础组件 | ClawWorks `src/theme/tokens.ts`、`components/layout/*` |
| `@dragon/studio` | 完整工作台 SPA（路由、页面、状态） | ClawWorks `App.tsx` 路由 + Loong `gateway-dashboard` 能力页 |
| `@dragon/desktop` | Tauri 工程、Rust Host 实现、安装包 | ClawWorks `src-tauri/*` |
| （已有）`@dragon/gateway` 等 | Kernel，不变 | Loong-Agent 现状 |

**退役路径**：`@dragon/gateway-dashboard` 页面迁入 `@dragon/studio` 后，仅保留构建嵌入或删除独立入口。
...
**P1 里程碑**：`pnpm --filter @dragon/client test` 通过；示例脚本可 `get` model config。

---

### P2 — UI 设计系统（`@dragon/ui`）
...
**P3 里程碑**：日常开发只用 `pnpm studio:dev` + `dragon gateway`，不再打开旧 dashboard。

---

### P4 — Desktop Surface（`@dragon/desktop`）
...
| P4-03 | Rust `config.rs`：Loong 路径解析 | P0-06 | `DRAGON_DATA_ROOT`、`.dragon` | 与 `dragon-paths.ts` 一致 |
| P4-04 | Rust `init.rs`：首次运行初始化 | P4-03 | 默认配置目录 | 幂等 |
| P4-05 | **Watchdog**：spawn `dragon gateway` | P4-03 | `watchdog.rs` | 健康检查、退避重启 |
...
| P4-10 | `createTauriHost()` 注入 Studio | P1-05,P4-06 | `@dragon/host/tauri` | capabilities 正确 |
...
| ID | 任务 | 依赖 | 交付物 | 验收标准 |
|----|------|------|--------|----------|
| P5-01 | Cloud API 模块（`@dragon/client/cloud`） | P1 | 登录、refresh、profile | 可配置 baseUrl |
| P5-02 | 登录页 UI | P3,P5-01 | `pages/Login.tsx` | JWT 写入 Host 安全存储 |
| P5-03 | `gateway-config` 拉取与合并 | P5-01,P4-11 | `applyRemoteGatewayConfig` | 写 `.dragon/config/providers.json` |
...
| P6-02 | 渠道中心 Channels | P4 | ClawWorks `commands/im.rs` → Dragon channels |
| P6-03 | 任务中心 / Cron UI | P3 | 对接 `@dragon/cron` RPC |
| P6-04 | Skills 市场 / 编辑器 | P3 | ClawWorks `SkillMarket`、`ContentEditor` |
| P6-05 | 分析页 Analytics | P3 | KPI / trajectory 可视化 |
| P6-06 | OpenClaw 配置迁移工具 | P4 | 一次性 import `openclaw.json` → `.dragon` |
...
| K-02 | `dragon-paths` 工作区根解析（已完成） | P4-03 |
...
|----------------|------------|------|
| `ClawWorks/src/theme/tokens.ts` | `@dragon/ui` | P2 |
| `ClawWorks/src/components/layout/*` | `@dragon/ui` | P2 |
| `ClawWorks/src/App.tsx` 路由壳 | `@dragon/studio` | P3 |
| `ClawWorks/src/pages/Chat.tsx` | `@dragon/studio` + Client SSE | P3 |
| `ClawWorks/src/services/gateway.ts` | `studio/services/gatewayReadiness.ts` | P3 |
| `ClawWorks/src/services/tauriCommands.ts` | `@dragon/host` + desktop IPC | P4 |
| `ClawWorks/src-tauri/src/watchdog.rs` | `packages/desktop/src-tauri` | P4 |
| `ClawWorks/src-tauri/src/config.rs` | `packages/desktop` + `dragon-paths` | P4 |
| `skills/clawworks-release-packager` | `packages/desktop/scripts` | P4 |
| `resources/runtime/openclaw` | **不迁移** → `dragon gateway` 子进程 | — |
...
pnpm --filter @dragon/studio dev

# Desktop（默认 Surface）
pnpm --filter @dragon/desktop tauri dev

# 发布
pnpm --filter @dragon/desktop release
```

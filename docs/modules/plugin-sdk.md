# @dragon/plugin-sdk 技术方案

## 1. 职责边界

**负责**：读取 `dragon.plugin.json`、安全加载插件 ESM 入口、在 `activate()` 期间收集 tools/providers/hooks/memory backends、规范化并冻结注册项。
...
| 导出 | 说明 |
|------|------|
| `loadDragonPlugin(root, options?)` | 返回 `LoadedDragonPlugin` |
| `DragonPlugin` / `DragonPluginManifest` | 插件契约 |
| `DragonPluginContext` | `registerTool`、`registerProvider`、`registerLifecycleHook`、`registerMemoryBackend` |
...
realpath(pluginRoot)
  ...
  → freeze 所有注册项
  → 返回 LoadedDragonPlugin { deactivate? }
```
...
1. 开发模式可选 `tsx` loader 或 `dragon plugin build` 脚手架。
2. Manifest 增加 `engines.dragon` semver 范围。

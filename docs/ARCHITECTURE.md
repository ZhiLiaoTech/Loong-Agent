...
- `POST /rpc` with `health`, `connect`, `agent`, run, provider, model config,
  agent config, plugin, tool, memory review, and trajectory request types
...
- `connect` capabilities include `events.websocket`, `providers.list`,
  ...
  `agent.config.get`, `agent.config.save`, `tool.invoke`, and
  `channels.webhook` when the surfaces are available
...
- `agent.config.get` and `agent.config.save` expose dashboard-editable Agent
  Profile configuration for default model/workspace/thinking and reusable

...
当前实现通过独立的 `shot-quality-response` 协议约束食物吸引力、动作明显度、产品露出和构图四个 0～1 分数，要求每个候选恰好返回一次结果。适配器具备传帧授权、路径边界、超时、重试、预算和调用指标保护；未授权或模型不可用时，由饱和度、曝光、清晰度、运动、机位角色、裁切安全和稳定性生成确定性后备分数。新分数进入总分，同时兼容读取不含这些字段的历史 1.0 候选产物。
...
`CVS-808` 已为视觉与文案适配器增加统一的逐次调用指标。记录包括操作类型、批次/尝试次数、成功/失败/超时/取消状态、输入输出计量单位、估算费用、耗时和错误码；使用 `CookingVideoMetricsStore.record` 后按作业追加到 `state/model-metrics.jsonl`。Gateway 的 `cooking.video.metrics.get` 返回总调用量、失败分类、平均/P95 耗时、估算费用、按模型操作聚合和流水线阶段耗时，Studio 在审核工作台中展示核心摘要。模型客户端当前未返回供应商账单 Token，因此视觉输入单位是关键帧数、文案输入输出单位是字符数，费用由部署方配置的单项/单次价格估算。

`CVS-809` 的数据采集与分析能力已经就绪。服务端在保存 EDL 时记录换机位、入出点、字幕、裁切、转场、增删片段、重排、音频和片尾修改计数；提交审核时将意见按节奏、机位、画质、文案、品牌、音频、合规、同步和其他原因分类。记录追加到 `state/human-feedback.jsonl`，不包含字幕正文、审核意见正文或视频内容。`cooking.video.feedback.summary` 支持单作业或当前 jobsRoot 下最多 500 个作业聚合，输出换机位率、修改次数、审核结果、返修模式和质检失败分布。Studio 展示单作业摘要。由于尚无客户真实审核数据，`CVS-809` 保持未完成，待试点数据进入后再形成有效业务结论。
...
- 每分钟素材的处理耗时。
- 代理生成、抽帧、模型分析和渲染耗时。
- 每个作业的模型调用数、输入输出计量单位、估算费用、平均/P95 耗时和失败分类。
...
具体字段、标注流程、复核问题码和评测匹配口径见 [黄金样本标注规范](./COOKING_PROMO_VIDEO_GOLDEN_ANNOTATION.md)。标注文件使用 `golden-annotation.schema.json`，并通过运行时校验补充检查跨字段引用、时间边界、禁用区间和最佳镜头一致性。
...
`CVS-901` 提供供应商无关的 `MultipartUploadCoordinator`。API 层只签发限时 PUT 地址，视频字节由客户浏览器或采集端直接写入对象存储，不经 Gateway 中转。对象 key 固定为 `tenantId/jobId/assetId/fileName`，各级 ID 和文件名先做路径安全校验。

创建会话时必须提交总字节数、整包小写 SHA-256 和分片大小。协调器将会话原子持久化到 `state/multipart/<uploadId>.json`，因此进程重启后可恢复已验证分片。每个分片先登记预期大小和摘要，再由对象存储适配器回查实际大小、SHA-256 与 ETag；全部连续分片验证通过后才允许合并。完成时再次核对对象 key、总字节数和整包 SHA-256，任何不一致都以 `UPLOAD_INTEGRITY_FAILED` 终止，不能把对象交给后续媒体流水线。
...
`CVS-903` 的 `ArtifactRetentionPolicy` 分别配置原素材 `input/`、代理 `proxy/`、抽帧 `frames/` 和成片 `output/` 的保留天数，`null` 表示永久保留，合法范围为 0～3,650 天。分析结果、EDL、作业状态和审计不在自动删除范围内，以保留可解释和可重渲染依据。

`enforceJobRetention` 默认只执行 dry-run 并返回候选路径、大小、修改时间和到期时间。物理删除必须同时传入 `dryRun: false` 和精确确认词 `DELETE_EXPIRED_ARTIFACTS`；运行中作业一律跳过，`legalHoldJobIds` 中的作业无论到期与否一律保留。删除前重新检查目标是作业根目录内的普通文件且不是符号链接；先写 planned 审计，再逐个删除并写 applied 汇总，即使执行中断也能追踪删除意图。

客户未冻结保留周期时只运行 dry-run。生产调度建议每日低峰执行，并在正式删除前审阅候选体积；对象存储侧应将同一策略映射为生命周期规则，同时保留本服务的执行审计。

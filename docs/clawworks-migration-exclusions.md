...
| 排除项 | 原因 | Loong 替代 |
|--------|------|------------|
| `runtime/openclaw/openclaw.mjs` 捆绑树 | 内核不同 | `loong gateway` 子进程 / 捆绑 `@loong/cli` |
| `openclaw.json` 作为主配置根 | 数据模型不同 | `.loong/config/*.json` |
| OpenClaw ACP 协议假设（端口 18789） | 默认端口与 RPC 不同 | Gateway HTTP RPC + SSE（17357） |
| `clawworks-model-gateway` 插件 | 云代理专用 | Loong providers + 可选云端下发 |
| ClawWorks 云端 API 硬编码 | 产品后端不同 | 可配置 `LOONG_CLOUD_URL`（P5） |
| Suite 安装器 / `instances/` 全量逻辑 | 路线图 P6 | 分期迁入 |
| Tauri 内 `invoke` 散落调用 | 应经 `@loong/host` | Host 能力接口 |

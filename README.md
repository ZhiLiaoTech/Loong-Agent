...
> 潜龙让您在自家环境里，用得上、管得住、换得了模型的 AI 帮手——少踩「演示很美、上线很惨」的坑。

相对「只有聊天页的 Bot」或「只能在终端里用的编码助手」，潜龙把能力连成一条**在您掌控下、可审计**的本地链路。下面说明**对您意味着什么**（实现细节见 [模块文档](docs/modules/README.md)）。

#### 客户最容易感受到的三点

1. **多模型分级动态调度** 支持按任务复杂度做多模型分级调度：简单任务优先走更快、更省的模型，复杂任务再切到更强的模型，并可配置失败回退。整体成本更低、响应更快，但关键任务质量依然稳得住。
2. **高效任务循环** 围绕任务持续收集上下文、执行动作、检查结果，持续循环推进。意味着需求从“问答”升级为“交付”，减少人工反复盯过程、补步骤、催结果。
3. **分层记忆和进化** 支持持续记忆与长会话压缩，同时把长期记忆做成可审核、可晋升的机制。减少重复解释、重复交代背景，另一方面也足内部合规和风险控制要求。

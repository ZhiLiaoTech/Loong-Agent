---
name: clawworks-content-creator-scheduler-v1.0.0
...
06:00 开始
  ...
  │   └── 未完成 → 使用昨日数据 + 标注「数据未更新」
  │
  ├── 调用 clawworks-content-creator-topic-radar-v1.0.0：生成今日推荐选题
  │   └── 输出 → output/topics.json
  │
  ├── 从 Top 5 选题中选取 2-3 个最优的
  │
  ├── 对每个选中的选题，调用 clawworks-content-creator-copywriter-v1.0.0：生成完整草稿
  │   ├── 标题 × 3 + 正文 + 标签 + 封面建议

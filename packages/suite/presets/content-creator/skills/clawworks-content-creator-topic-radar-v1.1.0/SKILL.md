---
name: clawworks-content-creator-topic-radar-v1.1.0
...
先检查 `output/profile.json` 与 `soul/USER.md` 是否完整；若画像缺失、过旧或置信度偏低，优先调用 `clawworks-content-creator-account-profiler-v1.1.0` 补齐画像。

随后读取 `USER.md` / `output/profile.json` 获取用户定位，调用 `scripts/trend_monitor.py --match` 执行：

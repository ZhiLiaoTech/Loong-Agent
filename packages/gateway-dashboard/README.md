# @loong/gateway-dashboard

> **Deprecation:** New UI work belongs in [`@loong/studio`](../studio/README.md).  
...
After **2026-08-31**, this package remains buildable for smoke tests only; `GET /` serves `@loong/studio` exclusively.

## Preferred: Loong Studio

| | Studio | gateway-dashboard (this package) |
|---|--------|----------------------------------|
| Dev | `pnpm studio:dev` → :1420 | `pnpm --filter @loong/gateway-dashboard dev` |
...
```bash
corepack pnpm --filter @loong/gateway-dashboard dev
corepack pnpm --filter @loong/gateway-dashboard build
```

`@loong/gateway` 在 `LOONG_UI=dashboard` 时读取 `dist/index.html`（内联单文件 HTML）。

## 认证

V2 使用 `SecretProvider` + sessionStorage key `loong.gateway.secret`（与 Studio 对齐）。

## 测试

```bash
corepack pnpm --filter @loong/test-suite test
```
